...
public class CalculatorUtils {
  @Autowired private BaihuaService baihuaService;
  @Autowired private QingyangService qingyangService;
  @Autowired private Luoyesong luoyesong;
  @Autowired private YunshanService yunshanService;
  ...
  public double getResults(EnvironmentalSilver environmentalSilver) {
    ...
    double zhishu =
        environmentalSilver.getLH() / (18400 * (1 + environmentalSilver.getT() / 273)); // 10上面的数
    ...
    zhongjian =
        4098
            * (0.618
                * Math.pow(
                    Math.E,
                    (17.27 * environmentalSilver.getT()) / (environmentalSilver.getT() + 237.3)));
    zhongjian =
        zhongjian / (environmentalSilver.getT() + 237.3) / (environmentalSilver.getT() + 237.3);
    ...
    x4 =
        (1
            + environmentalSilver.getRST()
                / (4.72
                    * (Math.log((environmentalSilver.getZ() - environmentalSilver.getD()))
                    / environmentalSilver.getZ0())
                    * Math.log(environmentalSilver.getZ() - environmentalSilver.getD())
                    / environmentalSilver.getZ0())
                / (1 + 0.54 * environmentalSilver.getU()));
